...
```
#### C语言三种常量类型的对比
##### 2.|C语言中字面量、#define、enum对比 3d
##### 4.|答：
1.字面量有其默认类型，也可通过后缀指定其类型
2.# define定义的宏常量可以是任意类型
3.enum定义的枚举常量只能是整型
#### C语言中的只读变量
##### 只读变量介绍
:PROPERTIES:
:custom_id: 5f7409e9-9db8-4490-80bc-0137874f3697
:END:
###### 2.| 什么是只读变量？
###### 4.|答：
1. 被const修饰的变量
###### 2. 只能作为右值使用
###### 3. 无法**直接**通过赋值改变const变量值
4. const修饰的变量不是真正意义上的变量
##### 只读变量的练习
###### 2.| 找出下列代码中的错误选项
```C++
int main()
...
```
###### 4.|答： 只读变量无法直接作为左值操作
###### 7.| ((5f7409e9-9db8-4490-80bc-0137874f3697)) ->只读变量介绍:+
## 第27课 字符数组与字符串（上）
### C语言中的字符串变量
#### 2.|字符串变量？
#### 4.| 答：
#### 1. 没有专用的字符串类型（字符：双引号括起来的有序字符集）
#### 2. 只能通过字符数组"模拟"字符串变量
#### 3.|存在字符串字面量，但仅能作为常量使用
### 字符串中的0元素（ascii表中的空）
#### 2.|字符串中的0元素？
#### 4.|答：
#### 1. 整数0（ascii中的0）表示空元素（空）
char c = 0; printf("%c",c);
#### 2. 0元素使用转义字符表示为'\0'
### “字符串变量” -- 字符数组
#### 2.|在C语言中如何使用“字符串变量”？
#### 4.| 答：
#### 1. C语言中通过字符数组定义字符串
#### 2. 当字符数组中**末尾**存在0元素（'\0'即空）时，可当做字符串使用
#### 3. 0元素表示了一个字符串的结束
#### 4. 一个字符数组不一定是字符串，而字符串一定是字符数组
```C++
    char b[] = { 'D', '.', 'T', '.' ,'\0'};
    char c[] = { 'D', '.','\0', 'T', '.' };
```
b是字符串，c不是
#### 5. 字符数组可以直接使用字符串常量进行初始化，char s[]=“hello”;char name[] = {"kewang"}; 但是**初始化后**不能再次**直接**赋值
## 第28课 字符数组与字符串（下）
### 获取字符串长度
#### 2.|C语言中获取字符串长度的方法是什么？
```C++
...
int main(){
    char s[10] = "abcd";
    int len = **kcode1::strlen(s);**
    return 0;
}
```
### 将s2（字符数组）中的字符赋值到s1 中 等价于 s1 = s2
#### 2.|如何s1 = s2 
```C++
#include<stdio.h>
#include<string.h>
